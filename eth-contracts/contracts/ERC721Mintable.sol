...
contract Pausable is Ownable{
    ...
    modifier paused(){
        require(_paused, "Contract has to be paused");
        _;
    }

    modifier whenNotPaused(){
        require(!_paused, "Contract has to be unpaused");
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to != address(0), "token cannot be minted to zero address");
    ...
    // @dev Internal function to burn an existing token
    function _burn(address owner, uint256 tokenId) internal {
        //Check if owner given is actual owner of token
        require(ownerOf(tokenId) == owner, "Can't burn token you don't own");

        //Clear all approvals
        _clearApproval(tokenId);

        //Decrement token count and transfer ownership to zero address
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "Cannot transfer token that is not owned");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        ...
        string memory stringTokenId = usingOraclize.uint2str(tokenId);
        _tokenURIs[tokenId] = usingOraclize.strConcat(_baseTokenURI, stringTokenId);
