...
contract SolnSquareVerifier is ERC721Mintable{
    ...
    require(solutionsSubmitted[solutionHash].solutionAddress == address(0), "Solution has already been added to solutionsSubmitted");

    bool isVerified = verifier.verifyTx(a,b,c,input);
    require(isVerified, "Solution can't be verified");

    // solution has be verified  add to solutions
    solutionsSubmitted[solutionHash] = Solution(_currentIndex.current(), msg.sender, false);
    solutions.push(Solution(_currentIndex.current(), msg.sender, false));
    // emit SolutionAdded
